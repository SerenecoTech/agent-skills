...
description: Rules for explanatory prose a person reads to understand something. Overviews, concept and architecture pages, rationale, release-note narrative, PR descriptions. Specific facts, plain verbs, no significance inflation, one new fact per sentence. Loaded by /humanize for descriptive sections. Invoke directly to rewrite an explanation that reads as filler.
...
For the complete guide with full examples, structural patterns, and the exhaustive vocabulary reference, read `references/reference.md` in this skill's directory.
