...
| `should`, `would`, `may`, `might`, `could`, `shall` | Banned modal (Rule 3.2) | See the modal ladder in SKILL.md. |
...
| `however`, `therefore`, `since` (= because), `now` | Common swap (SKILL.md) | but / thus, as a result / because / (delete) |
| `need to`, `have to` | Common swap | Imperative in procedures; `it is necessary to` in descriptive text |
| `perform`, `insert`, `reach`, `avoid`, `repeat`, `acceptable` | Common swap | do / put / get, get to / prevent / do … again / (give the limit) |
| `the example below`, `the section above` | "below" and "above" used as adverbs | Name the target, or write `…that follows` |
| ` is complete`, ` are complete` | "complete" used as an adjective | completed (adjective), or full / all |
...
Report style violations only. Do not report facts you believe are wrong — say so separately, outside the violation list.
